...
async fn test_program() {
    ...
    let data = State { counter: 0, f1: 0, f2: 0 }.try_to_vec().unwrap();
    ...
    for i in 0..10 {
        let init_data = InstructionData { f1: 18 + i, f2: 109 + i as u64 }.try_to_vec().unwrap();
    ...
    let data = state_account.data().as_slice();
    println!("z1: {:?}", data);
